...
class TaskRewardRevocationTest {
    ...
    fun setUp() {
        ...
        character = CharacterRepositoryImpl(
            ...
            abstinenceCalculator = AbstinenceCalculator(),
        ...
        complete = CompleteTaskUseCase(
            ...
            anchors,
            widgets,
            RefreshAchievementsUseCase(
                character = character,
                loader = AchievementLoader(context),
    ...
    fun `возврат задачи в работу снимает начисленные монеты`() = runTest {
        ...
        // Разовое достижение за первое закрытие остаётся, награда за задачу — нет
        val oneTimeBonus = coins() - start

        // Повторный цикл закрытие-возврат должен дать чистый ноль
        complete(taskId)
        assertTrue(coins() > start + oneTimeBonus)
        reopen(taskId)
        assertEquals(start + oneTimeBonus, coins())
    }
